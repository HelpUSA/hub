...
export const CorporateFooter: React.FC<CorporateFooterProps> = ({
  ...
  const texts = {
    pt: {
      ...
      address: 'Florida / Delaware, United States',
      terms: 'TERMOS DE USO',
      privacy: 'PRIVACIDADE',
      productsList: [
        'HelpUS AI Agent Studio',
        'HelpUS Voice TTS',
        'HelpUS FBA Suite',
        'HelpUS Search RAG',
        'NexoAI Neural Bot'
    ...
    es: {
      ...
      address: 'Florida / Delaware, Estados Unidos',
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 pb-12 border-b border-slate-800/80">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="space-y-3 pt-2">
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t.followUs}</h4>
              <div className="flex items-center gap-3">
                <a
                  href={`https://wa.me/${whatsappNumber}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-9 h-9 rounded-full bg-slate-900 border border-slate-800 text-slate-400 hover:text-emerald-400 hover:border-emerald-500/50 flex items-center justify-center transition-all shadow-md"
                  aria-label="WhatsApp"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-9 h-9 rounded-full bg-slate-900 border border-slate-800 text-slate-400 hover:text-pink-400 hover:border-pink-500/50 flex items-center justify-center transition-all shadow-md"
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-9 h-9 rounded-full bg-slate-900 border border-slate-800 text-slate-400 hover:text-blue-400 hover:border-blue-500/50 flex items-center justify-center transition-all shadow-md"
                  aria-label="LinkedIn"
                >
          ...
          <div className="lg:col-span-3 space-y-6">
            <div>
              <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-3">{t.productsTitle}</h4>
              <ul className="space-y-2 text-xs font-medium text-slate-400">
                {t.productsList.map((item, idx) => (
                  <li key={idx}>
                    <button onClick={() => onNavigateCategory('ia')} className="hover:text-cyan-400 transition-colors">
                      {item}
                    </button>
                  </li>
                ))}
            ...
            <div className="pt-2">
              <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-3">{t.solutionsTitle}</h4>
              <ul className="space-y-2 text-xs font-medium text-slate-400">
                {t.solutionsLinks.map((item, idx) => (
                  <li key={idx}>
                    <button onClick={() => onNavigateCategory('setoriais')} className="hover:text-cyan-400 transition-colors">
                      {item}
                    </button>
                  </li>
                ))}
          ...
          <div className="lg:col-span-3 space-y-6">
            <div>
              <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-3">{t.resourcesTitle}</h4>
              <ul className="space-y-2 text-xs font-medium text-slate-400">
                {t.resourcesLinks.map((item, idx) => (
                  <li key={idx}>
                    <button onClick={() => onOpenModal('institutional')} className="hover:text-cyan-400 transition-colors">
                      {item}
                    </button>
                  </li>
                ))}
            ...
            <div className="pt-2 space-y-2 text-xs text-slate-400">
              <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-3">{t.supportTitle}</h4>
              <p><strong className="text-slate-300">WhatsApp:</strong> <a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noopener noreferrer" className="hover:text-cyan-400 transition-colors">{whatsappFormatted}</a></p>
              <p><strong className="text-slate-300">E-mail:</strong> <a href={`mailto:${helpusEmail}`} className="hover:text-cyan-400 transition-colors">{helpusEmail}</a></p>
              <p className="text-[11px] text-slate-500 pt-1 font-medium">{t.address}</p>
            </div>
          </div>
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-white placeholder-slate-500 text-xs focus:outline-none focus:border-cyan-400 transition-all"
                />
                ...
                <label className="flex items-start gap-2 text-[11px] text-slate-400 leading-tight cursor-pointer pt-1">
                  <input
                    ...
                    className="mt-0.5 rounded bg-slate-900 border-slate-800 text-cyan-500 focus:ring-cyan-500"
                  />
                ...
                <button
                  type="submit"
                  className="w-full py-3 px-6 rounded-xl bg-slate-900 hover:bg-slate-800 text-white border border-slate-800 font-bold text-xs tracking-wider uppercase flex items-center justify-between transition-all group"
                >
